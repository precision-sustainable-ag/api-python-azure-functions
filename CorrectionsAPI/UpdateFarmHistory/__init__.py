...
class UpdateFarmHistory:
    def __init__(self, req):
        initial_state = initializer.initialize(
            route_params=["code"], body_params=["values"], req=req)
    ...
    def update_producer(self):
        update_prod_query = sql.SQL("UPDATE {table} SET {values} WHERE {identifiers}").format(
            table=sql.Identifier("farm_history"),
            values=sql.SQL(', ').join(
        ...
        if self.crown_cur.rowcount == 0:
            return func.HttpResponse(json.dumps({"status": "error", "details": "failed to update farm_history"}), headers=global_vars.HEADER, status_code=400)
        else:
            return func.HttpResponse(json.dumps({"status": "success", "details": "successfully updated farm_history for code {} to {}".format(self.route_params_obj["code"], json.dumps(self.body_params_obj["values"]))}), headers={'content-type': 'application/json'}, status_code=201)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        protocol_updater = UpdateFarmHistory(req)
