...
class GithubIssues:
    ...
    def __init__(self, req):
        ...
        #  setup connection
        self.auth0_connection = http.client.HTTPSConnection(
    ...
    def get_token(self):
        ...
        if self.DEBUG:
            logging.info("Auth 0 Management API Token = " +
                         auth0_token_json_data['access_token'] + '\n')
            logging.info("\n\n")
    ...
    def get_users(self):
        ...
        if self.DEBUG:
            ...
            for user in auth0_users_json_data:
                logging.info(user.get("nickname") + "'s token = " +
                             user.get("identities")[0].get("access_token"))
            logging.info("\n\n")
    ...
    def create_github_comment(self, issue_number, body):
        ...
        url = 'https://api.github.com/repos/%s/%s/issues/%s/comments' % (
            self.GITHUB_REPO_OWNER, self.GITHUB_REPO_NAME, issue_number)
    ...
    def logic_parser(self):
        ...
        if self.action == "create":
            # create issue
            status, response = self.create_github_issue(
                self.title, self.body, self.assignees, self.labels)
            if status:
        ...
        # if action is comment
            ...
            # create comment
            # send back response
            status, response = self.create_github_comment(
                self.number, self.comment)
            if status:
                # send back succsess response
                return func.HttpResponse(
                    # body may be unneeded
                ...
                return func.HttpResponse(
                    # body may be unneeded
                    body=json.dumps(
                        {"Message": "Could not create comment. Error: " + response.content.decode()}),
                    status_code=400,
                    headers=self.HEADER
                )
            ...
            if add_status and accept_status:
                return func.HttpResponse(
                    # body may be unneeded
                    body=json.dumps(
                        {"Add message": add_response, "Accept message": accept_response}),
                    status_code=201,
                    headers=self.HEADER
                )
            else:
