...
def fetch_biomass(affiliation, requested_site):
    ...
    if (resp_status):
        ...
        if len(site_biomass) > 0 and \
                pd.notna(site_biomass.iloc[0].get('uncorrected_cc_dry_biomass_kg_ha')):
            biomass_data['uncorrected_cc_dry_biomass_lb_ac'] = (
                biomass_data['uncorrected_cc_dry_biomass_kg_ha']*0.892179)
            site_year = site_biomass.iloc[0].get('year')
            biomass_data = biomass_data[biomass_data['year'] == site_year]
            biomass_data['Rank'] = (
                biomass_data['uncorrected_cc_dry_biomass_lb_ac'].rank(ascending=1))
            biomass_data.sort_values(
                by=['uncorrected_cc_dry_biomass_lb_ac'], inplace=True)
            site_biomass = (
                biomass_data.loc[biomass_data['code'] == requested_site])

            yaxis = biomass_data['uncorrected_cc_dry_biomass_lb_ac'].to_list(
            )
            ...
            plt.scatter(int(site_biomass.iloc[0].get("Rank")), site_biomass.iloc[0].get(
                "uncorrected_cc_dry_biomass_lb_ac"), color='red', s=50)
            plt.text(int(site_biomass.iloc[0].get("Rank")), site_biomass.iloc[0].get(
                "uncorrected_cc_dry_biomass_lb_ac"), requested_site)
            ...
            plt.clf()
            plt.close()
            return site_biomass.iloc[0].get("uncorrected_cc_dry_biomass_lb_ac"), \
